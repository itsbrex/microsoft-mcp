...
def template_dirs() -> list[Path]:
    ...
    user_root = Path(user_root_str) if user_root_str else Path.home() / ".config" / "microsoft-mcp" / "templates"
    dirs: list[Path] = []
    if user_root.exists():
    ...
    for cat_dir, source in _category_dirs(category):
        ...
        for yaml_file in sorted(cat_dir.glob("*.yaml")):
            ...
            try:
                ...
                continue
            placeholders = [p.get("name") for p in data.get("placeholders", []) if isinstance(p, dict)]
            templates.append({
                "name": tpl_name,
...
def load_template(category: str, name: str) -> dict[str, Any]:
    ...
    for cat_dir, _source in _category_dirs(category):
        ...
        if candidate.exists():
            ...
            if not tpl.get("name"):
                raise ValueError(f"Template '{name}' (category '{category}') missing required field: name")
            if not tpl.get("html_template"):
                raise ValueError(f"Template '{name}' (category '{category}') missing required field: html_template")
...
def validate_template_data(tpl: dict[str, Any], data: dict[str, Any]) -> list[str]:
    ...
    for ph in tpl.get("placeholders", []):
        if isinstance(ph, dict) and ph.get("required") and not data.get(ph.get("name", "")):
...
_PRE_RENDERED_KEYS: frozenset[str] = frozenset({
    "agenda_items", "interviewer_items", "focus_area_items",
    "action_item_list", "format_items", "meta_items",
    "candidate_info", "client_info_items", "severity_status_info", "notes_content",
})
...
def _render_list_items(items: list[str], numbered: bool = True, bullet: str = "•") -> str:
    rows: list[str] = []
    for i, item in enumerate(items, 1):
        esc = _escape(item.strip())
        rows.append(f"<tr><td>{i}. {esc}</td></tr>" if numbered else f"<tr><td>{bullet} {esc}</td></tr>")
    return "\n".join(rows)
...
def _render_conditional_sections(tpl: dict[str, Any], data: dict[str, Any]) -> dict[str, str]:
...
def _substitute_placeholders(html_str: str, data: dict[str, Any], tpl: dict[str, Any]) -> str:
    ...
    for src, (dest, numbered, bullet) in {
        ...
        if subs.get(src):
            subs[dest] = _render_list_items(_parse_comma_list(subs[src]), numbered=numbered, bullet=bullet)
